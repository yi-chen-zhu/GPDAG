...
beta = 0.5  ## beta only serves to fill the integrity of the codes and is not used (since truth is infinite smooth)
set.seed(77)
J = 11
j_mm = 8
n = 2^J+1
...
tau = 30
sig = 0.1
X_cov = cov_gaussian(X, X, tau=tau)
...
nu1 = 1.5  ## result parent set size 2
nu2 = 3.5  ## result parent set size 4
...
finf_records = matrix(NA,nrow=nj,ncol=3)
f2_records = matrix(NA,nrow=nj,ncol=3)
sig_records = matrix(NA,nrow=nj,ncol=3)
RunTime_records = matrix(NA,nrow=nj,ncol=3)
...
for (j in j_lst){
  ...
  mcmc_obj1 = mcmc(Xj, dagj1, Yj, log_tau_fun, tau_bound, sig2_prior, sig2_bound, nu=beta, tau=tau, sig=sig, cov_type="gaussian",
                  n_mcmc=n_mcmc, n_burn=n_burn)
  dagj2 = dag_norm2_cpp[1:(2^j+1)]
  mcmc_obj2 = mcmc(Xj, dagj2, Yj, log_tau_fun, tau_bound, sig2_prior, sig2_bound, nu=beta, tau=tau, sig=sig, cov_type="gaussian",
                   n_mcmc=n_mcmc, n_burn=n_burn)
  ...
  if (j <=j_mm){
    ...
    mj = round(log(2^j+1))*2  ## number of neighbors for Maximin DAG
    ...
    mcmc_obj_mm = mcmc(Xj_mm, dagj_mm_cpp, Yj_mm, log_tau_fun, tau_bound, sig2_prior, sig2_bound, nu=beta, tau=tau, sig=sig, cov_type="gaussian",
                       n_mcmc=n_mcmc, n_burn=n_burn)
  }
  ...
  f2_records[j_ind,1:2] = c(sqrt(sum((fhat_norm1_records[[j_ind]] - Yf_norm[1:(2^j+1)])^2)/(2^j+1)),
...
df_time = data.frame(logn=log(2^j_lst+1), Norming2=RunTime_records[,1], Norming4=RunTime_records[,2], Maximin=RunTime_records[,3])
...
# grid.arrange(p_est, p_time, nrow=1, widths=c(1,1.3))


##----------------------------prior approximation-------------------------------
jprior_lst = seq(5,J)
# X_cov_id = X_cov[ord_2_id,ord_2_id]
W22_table = matrix(NA, nrow=length(jprior_lst), ncol=3)
...
for (j in jprior_lst){
  ...
  chol_j_obj = DAG_Chol(X_norm, dag_j_cpp, nu=beta, tau=tau, cov_type="gaussian")
  ...
  if (j <= j_mm){
    ...
    dagj_mm_cpp = Rdag_to_Cppdag(dagj_mm)
    chol_j_mm_obj = DAG_Chol(Xj_mm_all, dagj_mm_cpp, nu=beta, tau=tau, cov_type="gaussian")
    ...
    X_cov_j_mm = cov_gaussian(Xj_mm_all,Xj_mm_all,tau=tau)
...
df_approx = data.frame(logn=log(2^jprior_lst+1), Norming2=W22_table[,1], Norming4=W22_table[,2], Maximin=W22_table[,3])
df_approx_melt = reshape2::melt(df_approx, id.vars='logn', variable.name="Method", value.name="W22", na.rm = TRUE)
p_approx =
